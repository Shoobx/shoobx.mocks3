...
class BotoTestCase(unittest.TestCase):
    ...
    def test_key_etag(self):
        ...
        self.assertEqual('"d32bda93738f7e03adb22e66c90fbc04"', resp["ETag"])
    ...
    def test_boto3_head_object_with_versioning(self):
        ...
        head_object = obj.meta.client.head_object(Bucket="mybucket", Key="hello.txt")
        ...
        self.assertEqual("0", old_head_object["VersionId"])
        self.assertEqual(len(old_content), old_head_object["ContentLength"])
    ...
    def test_multipart_upload_too_small(self):
        ...
        etags = [
            self.upload_part(upload_id, "the-key", body, pn)
            for pn, body in enumerate([b"hello", b"world"], 1)
        ]
    ...
    def test_multipart_upload(self):
        ...
        etags = [
            self.upload_part(upload_id, "the-key", body, pn)
            for pn, body in enumerate([part1, part2], 1)
        ]
        self.complete_multipart_upload(upload_id, "the-key", etags)
    ...
    def test_multipart_upload_out_of_order(self):
        ...
        etags = [
            self.upload_part(upload_id, "the-key", body, pn)
            for pn, body in [(4, part2), (2, part1)]
        ]
        self.s3.complete_multipart_upload(
            Bucket="mybucket",
            Key="the-key",
    ...
    def test_multipart_upload_with_headers(self):
        upload_id = self.create_multipart_upload("the-key", metadata={"foo": "bar"})
        part1 = b"0" * 10
    ...
    def test_boto3_multipart_etag(self):
        upload_id = self.s3.create_multipart_upload(Bucket="mybucket", Key="the-key")[
            "UploadId"
        ]
        part1 = b"0" * REDUCED_PART_SIZE
        etags = []
        ...
        self.assertEqual('"66d1a1a2ed08fd05c137f316af4ff255-2"', resp["ETag"])
    ...
    def test_list_multiparts(self):
        ...
        self.assertEqual(
            {"one-key": uid1, "two-key": uid2}, {u.object_key: u.id for u in uploads}
        )
        for u in uploads:
    ...
    def test_last_modified(self):
        ...
        self.assertEqual("2012-01-01 12:00:00+00:00", str(os[0].last_modified))
    ...
    def test_post_to_bucket(self):
        requests.post(
            "https://s3.amazonaws.com/mybucket", {"key": "the-key", "file": "nothing"}
        )
        body = self.retrieve_key("the-key")
    ...
    def test_post_with_metadata_to_bucket(self):
        ...
        self.assertEqual({"test": "metadata"}, self.bucket.Object("the-key").metadata)
    ...
    def test_delete_keys(self):
        ...
        rsp = self.s3.delete_objects(
            Bucket="mybucket", Delete={"Objects": [{"Key": "file2"}, {"Key": "file3"}]}
        )
    ...
    def test_delete_keys_with_invalid(self):
        ...
        rsp = self.s3.delete_objects(
            Bucket="mybucket", Delete={"Objects": [{"Key": "abc"}, {"Key": "file3"}]}
        )
    ...
    def test_bucket_key_listing_order(self):
        ...
        rsp = self.s3.list_objects(Bucket="mybucket", Prefix=prefix, Delimiter="/")
        self.assertEqual(
            ["toplevel/y.key1", "toplevel/y.key2", "toplevel/y.key3"],
            [x["Key"] for x in rsp["Contents"]],
        )
        self.assertEqual(["toplevel/x/"], [x["Prefix"] for x in rsp["CommonPrefixes"]])

        # Test delimiter with no prefix
        rsp = self.s3.list_objects(Bucket="mybucket", Delimiter="/")
        self.assertEqual(["toplevel/"], [x["Prefix"] for x in rsp["CommonPrefixes"]])
    ...
    def test_copy_key_reduced_redundancy(self):
        ...
        self.assertEqual(
            "REDUCED_REDUNDANCY", self.bucket.Object("new-key").storage_class
        )
        self.assertEqual("STANDARD", self.bucket.Object("the-key").storage_class)
    ...
    def test_acl_setting(self):
        ...
        self.assertTrue(
            any(
                ...
                and g["Grantee"]["URI"] == ALL_USERS_GRANTEE.uri
    ...
    def test_acl_switching(self):
        self.s3.put_object(
            Bucket="mybucket", Key="test.txt", Body=b"imafile", ACL="public-read"
        )
    ...
    def test_ranged_get(self):
        ...
        def contentsEqual(data, range):
            ...
            self.assertEqual(data, rsp["Body"].read())
    ...
    def test_policy(self):
        ...
        self.assertEqual(policy, self.bucket.Policy().policy)
    ...
    def test_website_configuration(self):
        ...
        self.assertEqual(cfg["IndexDocument"], self.bucket.Website().index_document)
        self.assertEqual(cfg["RoutingRules"], self.bucket.Website().routing_rules)
